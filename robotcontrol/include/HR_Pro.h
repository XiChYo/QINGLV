/*******************************************************************
 *	Copyright(c) 2020-2025 Huayan-Robotics Co., Ltd.
 *  All rights reserved.
 *	
 *	FileName:HR_Pro.h
 *	Descriptio:C++SDK
 ...
 *                                                                                                                                                                       MoveC_nJ
 * --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
 *   V1.0.15.0                       2025.11.19                                              HRIF_MoveC2
 *                                                                                           HRIF_SetReduceMode
 *                                                                                           HRIF_ReadReduceMode
 *                                                                                           HRIF_MoveLByLockAxis_nJ
...
/**
 *	@brief: Connect to the controller, start master, initialize slave, configure and check paramters, finally switch to DISABLE state / 连接控制器，连接过程中会启动主站，初始化从站，配置参数，检查配置，完成后跳转到去使能状态
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Set reduce mode / 设置缩减模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nReduceMode : Reduce mode, 0 to exit reduce mode, 1 to entry reduce mode / 缩减模式状态 0：退出缩减模式 1：进入缩减模式
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_SetReduceMode(unsigned int boxID, unsigned int rbtID, int nReduceMode);

/**
 *	@brief: Read reduce mode / 读取缩减模式
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nReduceMode : Current reduce mode, 0 for not reduce, 1 for reduce mode / 当前缩减模式状态 0：无缩减 1：缩减模式
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
HANSROBOT_PRO_API int HRIF_ReadReduceMode(unsigned int boxID, unsigned int rbtID, int& nReduceMode);
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param TCPList : vector of TCP list / 工具坐标的名称列表
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param TCPList : vector of UCS list / 用户坐标的名称列表
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param Linear : translation compliance / 平移柔顺度
 *	@param Angular : rotation compliance / 旋转柔顺度
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param nMode : speed mode, 0 as normal mode, 1 as slowmode, 2 as fast mode, 3 as Welding mode / 速度模式，0:正常模式，1:慢速精准模式，2:快速流畅模式, 3:焊接模式
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dMax :  Max tangential force (N) / 切向力最大值，单位：N
 *	@param dMin :  Min tangential force (N) / 切向力最小值，单位：N
 *  @param dVel :  Max lifting Velocity (mm/s) /越障上抬最大速度，单位：mm/s
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param dX-dZ :The vactor direction of compensation force based Base / 补偿力在基坐标系下的矢量方向
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dForceThreshold : Force activation threshold(N) / 自由驱动力启动阈值，单位：N
 *	@param dTorqueThreshold : Torque activation threshold (Nm) / 自由驱动力矩启动阈值，单位：N.m
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dMaxLinearVelocity : Max linear speed (mm/s) / 自由驱动最大直线速度，单位：mm/s
 *	@param dMaxAngularVelocity : Max attitude angular speed (°/s) / 自由驱动最大姿态角速度，单位：°/s
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param Dis_X-Dis_RZ : Max distance of exploration(mm) / 探寻最大距离，单位：mm
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param Pos_X-Pos_RZ : Max distance of positive (mm) / 正方向(与探寻方向一致)最大距离，单位：mm
 *	@param Neg_X-Neg_RZ : Max distance of negative (mm) / 负方向(与探寻方向相反)最大距离，单位：mm
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param dAngle : The angle at which the entire circle runs / 整圆运行的角度(如 dAngle = 420°，则运行一整圈圆过 60°)
 ...
 *           dRadius：Blending radius, mm / 过渡半径，单位：mm
 *           dLinearVel：Velocity for Path / 轨迹运动速度，单位：mm/s
 *           dLinearAcc： Acceleration for Path / 轨迹运动加速度，单位：mm/s^2
 *           dLinearJerk：Jerk for Path / 轨迹运动加加速度，单位：mm/s^3
 *           dUcs_X-dUcs_Rz：User coordinates / 用户坐标  
 *           dTcp_X-dTcp_Rz：Tool coordinates / 工具坐标  
 *           nRawPointCount：Raw points count / 原始点位个数
 *           HeadPoint[]：The first raw point coordinates (J1/J2/J3/J4/J5/J6 or X/Y/Z/RX/RY/RZ) / 第一个原始点位坐标(J1/J2/J3/J4/J5/J6 或 X/Y/Z/RX/RY/RZ)
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: Set the reference joint coordinates for space point trajectory/ 设置空间点位轨迹参考关节坐标
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param strPathName : Path name / 轨迹名称
 *  @param dJ1-dJ6 : Reference joint coordinates / 参考关节坐标
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: In SpeedJ mode,Push joint command velocity / 在SpeedJ模式下，下发关节命令速度指令
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dJ1CmdVel~dJ6CmdVel : Joint speed / 更新的命令关节速度，单位：°/s
 *  @param dAcc : Joint Acc / 关节设定加速度，单位：°/s^2
 *  @param dRuntime : Command execution exceeds this time, the movement will stop. / 指令运行超过该时间，运动将会停止。单位：s
 *	
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
...
/**
 *	@brief: In SpeedL mode,Push position command velocity / 在SpeedL模式下，下发空间运动命令速度指令
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 ...
 *	@param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 *	@param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
 *	@param dX~dZ: invalid, 0 recommended / 无意义的参数，建议输入0
 ...
 *  @param Display: display / 显示
 *  @param Switch: enable / 启用
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */
 ...
 *	@param nIsSeek,nIOBit,nIOState：Seeking or not, 1 for seeking; Motion stops when DO-nIOBit equals nIoState / 探寻参数，当nIsSeek为1，则开启探寻，这时电箱的DO nIOBit位为nIOState时，就停止运动，否则运动到目标点再停止
 *	@param strCmdID：Command ID, waypoint ID also, customized or set to 1, 2, 3 in order / 当前路点ID，可以自定义，也可以按顺序设置为“1”，“2”，“3”
 *  @param lockedFlags : Joint locking flags (bitmask). Bits 6-9 represent external axes J7-J10. Set bit to 1 to lock the corresponding axis, 0 to unlock.Example values: Lock J7: 0x40 (64 in decimal),Lock J7 and J8: 0xC0 (192 in decimal),Valid range: The joints that can be locked need to be determined by the specific controller version.
 *                       / 关节锁定标志（位掩码）。第6-9位对应外部轴J7-J10。置1锁定对应轴，置0解锁。示例值：锁定J7：0x40（十进制64）锁定J7和J8：0xC0（十进制192）合法范围：可以锁定的关节需要由具体的控制器版本决定。
 *	@return : nRet=0 : Function call succeeded / 函数调用成功
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_MoveLByLockAxis_nJ(unsigned int boxID, unsigned int rbtID, double dX, double dY, double dZ, double dRx, double dRy, double dRz,
                JointsData joints,
                string sTcpName, string sUcsName, double dVelocity, double dAcc, double dRadius,
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_MoveCByLockAxis_nJ(unsigned int boxID, unsigned int rbtID,
                double dStartPos_X,double dStartPos_Y,double dStartPos_Z,double dStartPos_Rx,double dStartPos_Ry,double dStartPos_Rz,
                double dAuxPos_X,double dAuxPos_Y,double dAuxPos_Z,double dAuxPos_Rx,double dAuxPos_Ry,double dAuxPos_Rz,
                double dEndPos_X,double dEndPos_Y,double dEndPos_Z,double dEndPos_Rx,double dEndPos_Ry,double dEndPos_Rz,
                int nFixedPosure,int nMoveCType,double dRadLen,double dVelocity, double dAcc, 
 ...
 *            nRet>0 : Error code of function call / 函数调用失败的错误码
 */	
HANSROBOT_PRO_API int HRIF_WayPoint2ByLockAxis_nJ(unsigned int boxID, unsigned int rbtID, int nMoveType,
                double dEndPos_X, double dEndPos_Y, double dEndPos_Z, double dEndPos_Rx, double dEndPos_Ry, double dEndPos_Rz,   
                double dAuxPos_X, double dAuxPos_Y, double dAuxPos_Z, double dAuxPos_Rx, double dAuxPos_Ry, double dAuxPos_Rz,                       
                JointsData joints,
                string sTcpName, string sUcsName, double dVelocity, double dAcc, double dRadius, 
...
/**
 * @brief Get calibration parameters/获取标定参数
 * @param boxID : Control box ID, 0 as default / 电箱ID号，默认值=0
 * @param rbtID : Robot ID, 0 as default / 机器人ID号，默认值=0
