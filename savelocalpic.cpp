...
void saveLocalpic::saveresultpicture(const QImage& img, const QString& fileName)
{
    times += 1;
    QImage imgReal = img;  // 把图像复制的代码放到这里来，不要放在相机线程
    imgReal.detach();  // 转换深拷贝
//    qDebug() << "saveResultPic times:" << QString::number(times);
    // 保存图片到本地saveResultPic文件夹
    QString baseDir = QCoreApplication::applicationDirPath();

    QString saveDirPath = baseDir + "/saveResultPic";
    QDir saveDir(saveDirPath);
    if (!dirInited)
    {
        if (!saveDir.exists())
