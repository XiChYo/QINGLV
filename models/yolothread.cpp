...
QPointF calcRelativeCenter(const cv::Rect& box, const std::vector<cv::Point>& contour, int imgW, int imgH)
{
    ...
    if (!contour.empty()) {
        ...
        if (std::abs(m.m00) > 1e-6) {
            ...
            return QPointF(clampValue(cx / safeW, 0.0, 1.0),
                           clampValue(cy / safeH, 0.0, 1.0));
        }
    }
...
QVector<QPointF> contourToRelativeSegmentation(const cv::Rect& box, const std::vector<cv::Point>& contour, int imgW, int imgH)
{
    ...
    if (!contour.empty()) {
        ...
        for (const cv::Point& p : contour) {
            ...
            segmentation.push_back(QPointF(clampValue(gx / safeW, 0.0, 1.0),
                                           clampValue(gy / safeH, 0.0, 1.0)));
        }
        return segmentation;
    }
...
bool yolothread::setEnabledClasses(const QList<int>& classIds)
{
    ...
    return setEnabledClassesInThread(classIds);
}
...
YoloTaskResult yolothread::predict(quint64 taskId, const QImage& image)
{
    ...
    return predictInThread(taskId, image);
}
...
void yolothread::run()
{
    ...
    bool loaded = false;
    {
        ...
        loaded = loadModelLocked(errorMessage);
        m_modelReady = loaded;
    }
    ...
    LOG_INFO("YOLO thread started and model loaded");
    exec();
...
{
    QMutexLocker locker(&m_mutex);
    return setEnabledClassesLocked(classIds);
}

YoloTaskResult yolothread::predictInThread(quint64 taskId, const QImage& image)
{
    QMutexLocker locker(&m_mutex);
    return predictLocked(taskId, image);
}

void yolothread::setEnabledClassesAsync(const QList<int>& classIds)
{
    const bool ok = setEnabledClassesInThread(classIds);
    emit setEnabledClassesDoneSig(ok);
}

void yolothread::predictAsync(quint64 taskId, const QImage& image)
{
    const YoloTaskResult result = predictInThread(taskId, image);
    emit predictDoneSig(result);
}

bool yolothread::loadModelLocked(QString& errorMessage)
{
    if (m_ctx != 0) {
        return true;
    }
...
void yolothread::releaseModelLocked()
{
    if (m_ctx != 0) {
        rknn_destroy(m_ctx);
        m_ctx = 0;
...
bool yolothread::setEnabledClassesLocked(const QList<int>& classIds)
{
    if (classIds.isEmpty()) {
        return false;
    }
...
YoloTaskResult yolothread::predictLocked(quint64 taskId, const QImage& image)
{
    ...
    const float scale = std::min(static_cast<float>(targetWidth) / bgrMat.cols,
                                 static_cast<float>(targetHeight) / bgrMat.rows);
    ...
    if (ret < 0) {
        taskResult.errorMessage = QString("rknn_inputs_set failed: ret=%1 input_type=%2")
                                      .arg(ret)
                                      .arg(tensorTypeToStr(input.type));
        return taskResult;
    }
    ...
    // Convert mask to relative segmentation points and center.
...
unsigned char* yolothread::loadModelData(const char* filename, int* modelSize)
{
